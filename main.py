...
def _custom_screener(sort_field, min_volume=50000, min_change=0.0):
    body = {
        "size": SCREENER_COUNT,
        "offset": 0,
        ...
        "query": {
            ...
            "operands": [
                ...
                {"operator": "LTE", "operands": ["intradayprice", MAX_PRICE]},
                {"operator": "GT", "operands": ["dayvolume", min_volume]},
                {"operator": "GT", "operands": ["percentchange", min_change]},
            ],
        },
        "userId": "",
        "userIdType": "guid",
    ...
    try:
        for attempt in range(YAHOO_RETRIES):
            r = HTTP.post(CUSTOM_SCREENER_URL, params=params, json=body, timeout=YAHOO_TIMEOUT)
            if r.status_code == 200:
                return r.json().get("finance", {}).get("result", [{}])[0].get("quotes", [])
            if r.status_code in (429, 500, 502, 503, 504) and attempt < YAHOO_RETRIES - 1:
                time.sleep(min(YAHOO_BASE_BACKOFF * (2 ** attempt) + random.random(), 8))
                continue
            logger.warning("Custom screener HTTP %s: %s", r.status_code, r.text[:200])
            break
    except Exception as e:
        logger.warning("Custom screener error: %s", e)
    return []
...
def get_active_universe():
    ...
    # 1) Yükselenler: erken momentum.
    quotes.extend(_custom_screener("percentchange", min_volume=50000, min_change=MIN_DAILY_CHANGE))
    # 2) En yüksek hacim: yükselişi henüz %2'ye ulaşmamış kaliteli adayları kaçırmamak için.
    quotes.extend(_custom_screener("dayvolume", min_volume=100000, min_change=0.0))

    # 3) Hazır small-cap gainers listesi; custom endpoint başarısız olursa yedek.
    data = _screener_get({
        "formatted": "false",
        "lang": "en-US",
        "region": "US",
    ...
    for q in quotes:
        ...
        price = q.get("regularMarketPrice", q.get("intradayPrice"))
        volume = q.get("regularMarketVolume", q.get("dayVolume", 0)) or 0
        change = q.get("regularMarketChangePercent", q.get("percentChange", 0)) or 0
        ...
        if not symbol or not (MIN_PRICE <= price <= MAX_PRICE):
            continue
        if volume * price < (PENNY_MIN_DOLLAR_VOLUME if price < 1 else MIN_DOLLAR_VOLUME):
            continue
        if change < MIN_DAILY_CHANGE and volume < avg_volume * 1.25:
            continue
        if avg_volume > 0 and volume / avg_volume < 0.75 and change < 5:
            continue
        ...
        if avg_volume > 0:
            ...
            if q_rvol >= 3:
                ...
                score += 8
        score += min(20, max(0, int((volume * price) / 1_000_000 * 5)))
        unique[symbol] = (score, q)

    ordered = [q for _, q in sorted(unique.values(), key=lambda x: x[0], reverse=True)]
    logger.info("Hızlı screener: %s aktif aday bulundu", len(ordered))
    return ordered
...
def analyze_swing(symbol, quote):
    try:
        price = float(quote.get("regularMarketPrice", quote.get("intradayPrice")))
        day_change = float(quote.get("regularMarketChangePercent", quote.get("percentChange", 0)) or 0)
        day_volume = float(quote.get("regularMarketVolume", quote.get("dayVolume", 0)) or 0)
    ...
    if price < ema20 or ema20 < ema50:
        return None
    if day_change < MIN_DAILY_CHANGE and price < resistance1 * 0.98:
        return None
...
def start_scheduler():
    ...
    # Yeni swing fırsatlarını 5 dakikada bir ara.
    scheduler.add_job(
        run_market_pipeline,
        CronTrigger(day_of_week="mon-fri", hour="4-19", minute="*/5"),
        id="swing_scan",
        replace_existing=True,
    )
    scheduler.start()
    logger.info("Scheduler başladı | 5 dk swing scan")
    return scheduler
